...
import {
  calculateLegBetPoints,
  calculateRaceBetPoints,
} from "../../utils/pointsCalculator";
import {
  constructLegEndMessage,
  constructRaceEndMessage,
} from "../../utils/messageConstructor";
...
const GameLogic = ({ setGameEnd }) => {
  ...
  const endLeg = () => {
    ...
    const messageAfterLeg = constructLegEndMessage(
      playerNames,
      legBetPoints,
      legRollPoints
    );
  ...
  const endGame = () => {
    ...
    const messageAfterRace = constructRaceEndMessage(
      playerNames,
      raceBetPoints
    );
  ...
  const checkAreAllDiceRolled = () =>
    !Object.values(dice).some((roll) => !roll);
